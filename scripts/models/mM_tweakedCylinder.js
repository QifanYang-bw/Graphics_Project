...
function maketweakedCylinder1() {
  ...
  var posArr = new Float32Array([0.0, 0.1, 0.2, 0.5, 0.7, 1.0]);
  var radArr = new Float32Array([0.8, 1.0, 0.8, 0.9, 0.9, 0.8]);

  var colorArr = [[0.9, 0.9, 0.9],
                  [0.7, 0.7, 0.7],
                  [0.9, 0.9, 0.9], 
                  [0.6, 0.6, 0.6], 
                  [0.5, 0.5, 0.5], 
  ...
  var capVerts = 24; // # of vertices around the topmost 'cap' of the shape
  ...
  for(v=0,j=0;   v<(2*capVerts) + 1;   v++,j+=g_floatsPerVertex) {  
    ...
    {       // put even# vertices around bottom cap's outer edge,starting at v=0.
      ...
      g_cylAry[j  ] = Math.cos(Math.PI*v/capVerts) * radArr[0];     // x
      g_cylAry[j + 1] = Math.sin(Math.PI*v/capVerts) * radArr[0];     // y
  ...
  for (var i = 0; i < nArr - 1; i++) {
    ...
    for(v=0; v< 2*capVerts + 2;   v++, j+=g_floatsPerVertex) {
      ...
      {   
          g_cylAry[j  ] = radArr[i] * Math.cos(Math.PI*(v)/capVerts);   // x
          g_cylAry[j + 1] = radArr[i] * Math.sin(Math.PI*(v)/capVerts);   // y
      ...
      {
          g_cylAry[j  ] = radArr[i + 1] * Math.cos(Math.PI*(v-1)/capVerts);   // x
          g_cylAry[j + 1] = radArr[i + 1] * Math.sin(Math.PI*(v-1)/capVerts);   // y
  ...
  for(v=0; v < (2*capVerts  + 1); v++, j+= g_floatsPerVertex) {
    ...
    if(v%2==0) {  // position even #'d vertices around top cap's outer edge.
      g_cylAry[j  ] = radArr[nArr - 1] * Math.cos(Math.PI*(v)/capVerts);   // x
      g_cylAry[j + 1] = radArr[nArr - 1] * Math.sin(Math.PI*(v)/capVerts);   // y
